  ...
  <div class="container">
    ...
    @foreach(App\Slide::all() as $slide)
        <?php if(!empty($slide->photoone) ) {?>
          <img  src="{{url('upload/slide/'.$slide->photoone)}}" style="width: 1200px; height: 380px">
        </div>

        <div class="item">
        <?php }elseif(!empty($slide->phototwo)) {?>
            <img  src="{{url('upload/slide/'.$slide->phototwo)}}" style="width: 1200px; height: 380px">
        </div>

        <div class="item">
        <?php }elseif(!empty($slide->photothree)) {?>
            <img  src="{{url('upload/slide/'.$slide->photothree)}}" style="width: 1200px; height: 380px">
        </div>
        <div class="item">
       <?php }elseif(!empty($slide->photofour)){ ?>
            <img  src="{{url('upload/slide/'.$slide->photofour)}}" style="width: 1200px; height: 380px">
        </div>
       <?php   }?>
   @endforeach
        </div>
    ...
    <!-- =============================== Start Top Seller ======================== -->
        ...
        <hr>
            ...
            @foreach(App\Product::latest()->paginate(8) as $product)
            ...
            @foreach(App\Product::all()->where('price', '>', '2000') as $product)
                <a href="{{url('/allcategories/'.$product->id.'/view')}}">
                    <div class="card">
                        <span>{{$product->price}}$</span>
                        <img src="{{url('/upload/product/'.$product->photo)}}">
    ...
    <!-- =============================== Start About   ======================== -->
                   <!-- <div class="about" id="about">
                        <div>
                            <h1>{{getsetting('About heading')}}</h1>
                        </div>
                        
                        <p>{{getsetting('About Body')}}</p>
                    </div> -->
    <!-- ===============================  End About   ======================= -->
    <!-- =============================== Start Brands ======================== -->
